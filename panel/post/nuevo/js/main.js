$(document).ready(function(){
    ...
    function publicar_post(titulo,url,contenido, referencias, etiqueta ) {
    	jQuery.ajax({
            url: "php/crear_post.php",
            type: "POST",
            data:{
            	Titulo: titulo,
            	Url: url,
		    ...
		    $.ajax({
		        ...
		        success: function(url) {
		           //var image = $('<img>').attr('src', 'http://' + url);
		            ...
		            $('#contenido').summernote("insertNode", image[0]);
		        },
		        error: function(data) {
		            console.log(data);
		        }
		...
		function sendFile(file, editor, welEditable) {
				...
				$.ajax({
					 ...
					 success: function(url) {
							 ...
							 $('#contenido').summernote('insertImage', url);
					 }
			 });
	    }
    $("#publicar").click(function(event) {
    	var url = $("#imagen_url").val();
    	var titulo = $("#titulo").val();
    	var contenido = $("#contenido").val();
    	var referencias = $("#referencias").val();
    	var etiqueta = $("#etiqueta").val();
			console.log(url);
			publicar_post(titulo,url,contenido, referencias, etiqueta);
    });
